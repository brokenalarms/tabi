 ...
 *
 * Usage:
 *   npx playwright test scripts/screenshots.ts --config playwright.config.ts
 *
...
<head><style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  body { width: 1024px; height: 768px; font-family: -apple-system, BlinkMacSystemFont, sans-serif; }
</style></head>
<body>${bodyHTML}</body>
</html>`);
...
test("screenshot: hint mode", async ({ page }) => {
  ...
  await setupPage(page, `
    <nav style="display:flex; gap:16px; padding:16px; background:#f5f5f5; border-bottom:1px solid #ddd;">
      <a href="/home" style="padding:8px 16px; color:#333; text-decoration:none;">Home</a>
      <a href="/about" style="padding:8px 16px; color:#333; text-decoration:none;">About</a>
      <a href="/docs" style="padding:8px 16px; color:#333; text-decoration:none;">Docs</a>
      <button style="padding:8px 16px; margin-left:auto;">Sign In</button>
    </nav>
    <main style="padding:32px;">
      <h1 style="font-size:24px; margin-bottom:16px;">Welcome</h1>
      <p style="margin-bottom:16px;">Navigate the web with your keyboard.</p>
      <div style="display:flex; gap:12px;">
        ...
        <a href="/learn" style="padding:10px 20px; border:1px solid #ccc; border-radius:6px; text-decoration:none; color:#333;">Learn More</a>
      </div>
    </main>
  `);
...
test("screenshot: help overlay", async ({ page }) => {
  ...
  await setupPage(page, `
    <div style="padding:32px;">
      <h1 style="font-size:24px;">Some page content</h1>
      <p>The help overlay appears in the bottom-right corner.</p>
    </div>
  `);
...
test("screenshot: tab search", async ({ page }) => {
  ...
  await setupPage(page, `
    <div style="padding:32px;">
      <h1 style="font-size:24px;">Background page</h1>
      <p>The tab search modal appears centered at the top.</p>
    </div>
  `);
